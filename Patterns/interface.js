...
var Interface = function(name, methods) {
    if (arguments.length !== 2) {
        throw new Error("Interface constructor must be called with "
...
Interface.ensureImplements = function(object) {
    if (arguments.length < 2) {
        throw new Error("ensureImplements must be invoked with "
                + "at least two params");
    }
    ...
    for (var i = 1, len = arguments.length; i < len; i++) {
        ...
        if (interface.constructor !== Interface) {
            throw new Error("ensureImplements expect arguments two and abowe "
